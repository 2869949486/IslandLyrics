...
swift run NotchLyricsCoreTests # 跑单测（无 XCTest 的断言式 runner）
```

## 源码结构

- `Sources/NotchLyricsCore/` — 纯逻辑 + 网络层（无 SwiftUI），可单测：PositionEngine(插值)、CDPPositionReader、AlgerMusicClient、NeteaseLyricParser、LyricIndex、PortDiscovery、ProcessDetector、Models、IslandState、ColorComponents。
- `Sources/NotchLyrics/` — AppKit/SwiftUI：IslandView(收起/展开状态机)、ExpandedView(seek/控制)、NotchWindow(覆盖窗)、NotchShape、PlayerStore(协调器)、StatusBarController、Settings*。
- `Tests/NotchLyricsCoreTests/` — 断言式测试 runner。
