...
<img src="assets/icon.png" width="120" alt="NotchLyrics" />
...
- 🎵 **从刘海自然长出的灵动岛**：顶贴边 + 肩部凹角 + 底圆角的 NotchShape，不是普通圆角浮窗；摄像头禁区任何状态都留空。
...
> NotchLyrics 本身**不播放音乐**，它是 AlgerMusic 的「歌词显示外设」——播放/控制/歌词都来自 AlgerMusic 的本地接口。
...
4. 在 **AlgerMusic → 设置 → 应用设置 → 远程控制** 里**开启远程控制**（默认是关的，NotchLyrics 取数据必需）。
5. 放首歌，刘海上就出现歌词岛了。
...
```bash
git clone https://github.com/2869949486/NotchLyrics.git
cd NotchLyrics
./build.sh                    # swift build -c release → 组 dist/灵动岛歌词.app → ad-hoc 签名
open "dist/灵动岛歌词.app"
pkill -x NotchLyrics          # 停止
```

跑测试：`swift run NotchLyricsCoreTests`（无 XCTest 依赖的断言式 runner）。
...
AlgerMusic 在本机暴露两个 localhost HTTP 口 + 一个可选的调试口，NotchLyrics 全靠它们：
...
> 技术栈：Swift / SwiftUI / AppKit，`NSPanel(.nonactivatingPanel)` 覆盖窗 + 自绘 NotchShape；数据/解析/插值在纯逻辑 Core 库里、有单测覆盖。
...
- **远程控制需手动开**：AlgerMusic 的远程控制默认关闭，需在其设置里开启（NotchLyrics 无法代开，会图文引导）。

---

## 🙏 致谢 / 关系说明

- **[AlgerMusicPlayer](https://github.com/algerkong/AlgerMusicPlayer)** by [@algerkong](https://github.com/algerkong)：本项目的数据源。NotchLyrics 只调用它在 localhost 暴露的 HTTP 接口，**不包含、不修改其任何代码**，二者各自独立、各自授权。
